...
static CFG_SDK_T gstSDKConfig =
{
	...
	.CFG_LOG_TO =
	{
		.eLogState = LOG_ENABLED,		// The default state of the lOG system. LOG_STANDBY: Locked LOG system (need to log in), LOG_ENABLED: Enabled LOG system, LOG_DISABLED: Disabled LOG system (can never be enabled)
        .pcPassword = "+88635786005",	// which is the password to login the LOG system
	},
	...
	.CFG_PLAYER_TO =
	{
		...
		| CFG_FILTER_AAC | CFG_FILTER_WMA | CFG_FILTER_AC3
		...
		| CFG_FILTER_AIFC | CFG_FILTER_DSD,
	...
	{
		.Enable = 1,
		.Func = EN_ADC_KEY_FUNC,		// EN_ADC_USER_FUNC: for key test, EN_ADC_KEY_FUNC: for key application
		.LevelNum = ADC_LEVEL_NUM_MAX,	// the max number of ADC level
		.pLevelTab = ADC_DataLevelTab, 	// the ADC level table defined in adc_KeyTab.inc
	},
	...
	.CFG_IR_TO =
	{
		.Cfg_Enable = 1,                    // 0: disable, 1: enable IR function
		.IopWakeUp = 1,                     // the CPU wake up system from standby mode by checking IR iPowerKeycode // 1: IOP(recommand), 0: Risc
		.TV_Remote_Ready_Enable = 1,        // 0: only the IR cmd from the main IR is active, 1: support the IR cmd from multi ir
		.MainIrFmt = 0,                     // which ir fmt is active in standby mode (check its power key) // 0: UPD6121 (NEC), 1: TC9012, 2: RC6 (philip)
		.IR_Learning_Enable = 1,            // 0: disable, 1: enable IR learning function
		.lCustomerID = IR_CUSTOMER_CODE,    // the customer ID for main IR, and the default fmt is NEC(sunplus IR:400)
		.iPowerKeycode = IR_POWER_KEY_CODE, // the power keycode for main IR, and the default fmt is NEC(sunplus IR:400)
	},
	//configure option for key polling process
	.CFG_KEY_TO =
	{
		.Cfg_Enable = 1,											// 0: disable, 1: enable key polling
		.iPollingTime = KEY_SCAN_FREQ, 								// the scanning frequency of a key (unit: 100ms)
		.iKeyTickFreq = KEY_TICK_TIME / KEY_SCAN_FREQ, 				// the tick frequency of a key (the interval of a tick)
		.iLongPressTickNum = KEY_LONG_PRESS_TIME / KEY_SCAN_FREQ,	// the tick for determining whether the key is long pressed
	},
	//configure option for CEC module
    .CFG_CEC_TO =
    {
        .CEC_HPD_WakeUp            = 0,// 1 to enable HPD wake up feature, 0 to disable
        .Cfg_CecAlwaysRun          = 1,// 1 CEC Running  in other Source ,0 CEC running in ARC or HDMI TX  source
    #ifdef USERAPP_SUPPORT_HDMI_CEC
        .Cfg_Enable                = 1, // 1 support HDMI CEC  CMD , 0 not support CEC  CMD
        .pbI2C_DevStr              = "i2c3_sw",// read EDID I2C device name
        .repeaterAddr              = 0x00,//EDID I2C device address
        .repeaterIntrGpio        = 25,// HPD check GPIO
    #else
        // ep repeater
        .Cfg_Enable                = 0,////SPA100 support HDMI CEC  CMD ,0 not support, but repeater self-support  CEC CMD set '0'
        .pbI2C_DevStr              = "i2cm_hw",// read EP register HW I2C device name
        .repeaterAddr              = 0xC2,//EP Register I2C device address
        .repeaterIntrGpio        = 25,//EP Interrupt GPIO
    #endif
	...
	.CFG_HDMI_TO =
	{
		.Cfg_HdmiOutputMode = eHDMI_AUTO,//eHDMI_480P, //eHDMI_576P,//eHDMI_AUTO,
	},
...
/*********************************************************************************************************/
    ...
    .CFG_PMC_TO =
    {
        ...
        .wIRwakeupMode     = 1,// 0: disable ir wakeup 1: enable ir wakeup 2: ir wakeup by user cb function
    ...
    // GPIO30
    .CFG_STDBY_GPIO_TO[5] = {
        .gpioChkEnable = 1, .gpioTrigLvl = 0, .functionEnable = SRC_PWRKEY
    },
	//configure option for PMC
	.CFG_POWER_KEY_TO =
	{
		.Cfg_Enable = 1,// enable pwrkey detect in normal mode. it does not work in standby mode
	},
	...
	.CFG_AUD_TO =
	{
		...
		/* {d0_left, d0_right, d1_l, d1_r, d2_l, d2_r, d3_l, d3_r, d4_l, d4_r, d5_l, d5_r, d6_l, d6_r, d7_l, d7_r}
		   ...
		   */
#if 1 //def SB714
		.Channel_Map = {L, R, Rrs, Rtr, Sub, C, Lrs, Ltr, Ltf, Rtf, Ls, Rs, Lw, Rw, Ltm, Rtm},
#else
		.Channel_Map = {L, R, Rs, Ls, Sub, C, Ltf, Rtf, Lrs, Rrs, Ltr, Rtr, Lw, Rw, Ltm, Rtm},
#endif
