...
/*SPA300 GPIO[55]-->102号引脚，用于控制蓝牙电源的开关*/
#define BT_POWER_PIN	 55
/*SPA300 GPIO[54]-->101号引脚，用于控制蓝牙的使能脚*/
#define BT_EN_PIN	54
...
static int ATS2819Mcu_SendCmd(BYTE func, BYTE cmd, BYTE len, BYTE *data, BYTE mode)
{
	...
	else
	{
		...
		/*数据的长度加上除去数据之外的5个字节的长度就是命令包的总长度*/
		ATS2819.len = len+5;
		for(i=0; i<len; i++)
		{
...
static int ATS2819_MCU_Init(QActive *me)
{
	...
	Pinmux_GPIO_Free(BT_EN_PIN);
	Pinmux_GPIO_Alloc(BT_EN_PIN,IO_GPIO,IO_RISC);
	GPIO_OE_Set(BT_EN_PIN, IO_OUTPUT);
	GPIO_Output_Write(BT_EN_PIN, IO_HIGH);
...
int ATS2819_MCU_Key(QActive *me, int key, int para)
{
	...
	switch(key)
	{
		...
		case SOURCE_SW_RELEASE_SIG:
			...
			/*如果是BT模式*/
			if(handle->src == BT_MODE_BT || handle->src == BT_MODE_USC)
			{
				/*打开BT的电源*/
