...
static void PowerOffAD82128(void)
{
	...
	Pinmux_GPIO_Free(12);
	Pinmux_GPIO_Alloc(12, IO_GPIO, IO_RISC);
	GPIO_OE_Set(12, IO_OUTPUT);
	GPIO_Output_Write(12, IO_LOW);

		Pinmux_GPIO_Free(13);
	Pinmux_GPIO_Alloc(13, IO_GPIO, IO_RISC);
	GPIO_OE_Set(13, IO_OUTPUT);
	GPIO_Output_Write(13, IO_LOW);

		Pinmux_GPIO_Free(6);
	Pinmux_GPIO_Alloc(6, IO_GPIO, IO_RISC);
	GPIO_OE_Set(6, IO_OUTPUT);
	GPIO_Output_Write(6, IO_LOW);

		Pinmux_GPIO_Free(5);
	Pinmux_GPIO_Alloc(5, IO_GPIO, IO_RISC);
	GPIO_OE_Set(5, IO_OUTPUT);
	GPIO_Output_Write(5, IO_LOW);
}
...
static void SetAD82128Output(const AmpList *const pstAmpList, eAmpIdx eIdx)
{
	...
	/*将33个寄存器的值写入到左右功放中，从而达到对左右声道以及中置声道功放的控制*/
	for(i=0; i<AMP_INIT_MAX; i++)
	{
#if 1 //def SB714
		switch (eIdx)
		{
			case eAmp1:AD82128Write(pstAmpList->peList[eIdx], AMP_INIT_LR[i][0], &AMP_INIT_LR[i][1], 1);break;
			case eAmp2:AD82128Write(pstAmpList->peList[eIdx], AMP_INIT_C[i][0], &AMP_INIT_C[i][1], 1);break;
			case eAmp3:AD82128Write(pstAmpList->peList[eIdx], AMP_INIT_FTLR[i][0], &AMP_INIT_FTLR[i][1], 1);break;
			case eAmp4:AD82128Write(pstAmpList->peList[eIdx], AMP_INIT_FSLR[i][0], &AMP_INIT_FSLR[i][1], 1);break;
			default:break;
		}
#else
		if(eIdx == eAmp1)  // LR
		{	
			/*将跟左右声道相关的寄存器的值写入到控制左右声道的输出的功放中*/
			AD82128Write(pstAmpList->peList[eIdx], AMP_INIT_LR[i][0], &AMP_INIT_LR[i][1], 1);
		}
		else if(eIdx == eAmp2)  // C
		{
			/*将跟中置声道相关的寄存器的值写入到控制中置声道的输出的功放中*/
			AD82128Write(pstAmpList->peList[eIdx], AMP_INIT_C[i][0], &AMP_INIT_C[i][1], 1);
		}
#endif
	}
	/*根据eIdx的索引号得到左右或者中置的寄存器的限幅值，然后将该限幅值写到对应功放的寄存器中*/
	setting = DRC_Map[eIdx];
	for(i=0; i<CHANNEL_SETTING_MAX; i++)
	{
		/*从0X1D寄存器开始，每个寄存器依次写入5个setting指向的数据*/
		AD82128Write(pstAmpList->peList[eIdx], 0X1D, (void *)(setting+CHANNEL_SETTING_DATA * i), CHANNEL_SETTING_DATA);
		/*启用将单个系数写入 RAM(0x32寄存器的[1]位)*/
...
#endif	
	/*设置完AD82128的输出状态之后延时100ms，让设置生效*/
	delay_1ms(100);
