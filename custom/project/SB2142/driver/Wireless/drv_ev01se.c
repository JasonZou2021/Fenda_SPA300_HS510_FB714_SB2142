...
#define CCH_10_MUTE_EN 			(1<<7)
#define CCH_10_MUTE 				(1<<6)
#define CCH_10_MUTE_AMP			(1<<5)
#define CCH_10_VOL_EN				(1<<4)
#define CCH_10_VOL_AMP				(1<<3)
#define CCH_10_EQ_EN				(1<<2)
#define CCH_10_AMP_PDN_EN			(1<<1)
#define CCH_10_AMP_PDN_H			(1<<0)
...
int EV01SE_Write(int iAddr, UINT8 *bData, int len)
{
	...
	if(ret != eI2COK)
	{
		LOGD(" EV01SE_Write reg 0x%x,  data 0x%x NG,  ret = %d\n ", iAddr, *bData, ret);
		return -1;
	}
	else
	{
...
int EV01SE_Read(int iAddr, UINT8 *bData, int len)
{
	...
	{
		LOGD(" EV01SE_Read reg %x  data %x NG\n ", iAddr, *bData);
		return -1;
	}
	else
	{
...
int EV01SE_Init(void)
{
	...
	EV01SE_Write(EV01SE_REG_LOST_SLEEP, &para, sizeof(para));
...
int EV01SE_Mute(BYTE mute)
{
	...
	if(mute == 1)     //mute
	{
		para[0] = CCH_10_MUTE_EN|CCH_10_MUTE|CCH_10_MUTE_AMP;
		para[1] = 0;
		para[2] = 0;
	}
	else
	{
		para[0] = CCH_10_MUTE_EN|CCH_10_MUTE_AMP;
		para[1] = 0;
		para[2] = 0;
	}
...
int EV01SE_Polling(void)
{
	...
	switch(pEV01SE->status)
	{
		...
		case EV01SE_POLL_CHECK_PAIR:
			...
			//	EV01SE_Read(EV01SE_REG_SYS_STATUS, &data1, sizeof(data1));
			
				if(EV01SE_Read(EV01SE_REG_CCH_RECV, data, sizeof(data)) == 0)
				{
					if(data[0] & 0x01)
					{
						pEV01SE->sub_syn =1 ;
					...
					//LOGD(" sub %x-%x-%x-%x-%x  [%x]\n", data[0], data[1], data[2], data[3], data[4], data1);
				}
...
static int EV01SE_Upgrade(QActive *me, ARFunc_t percent_func, eWirelessUpg type)
{
	...
	#define EV21SC_EEPROM_UPG_PERCENT_PER	( EV21SC_END_ADDR/100)
