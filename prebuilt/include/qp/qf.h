...
#define MAX_AO_NAME_LEN 24

/****************************************************************************/
typedef struct QMActive_s QMActive;
...
{
	int sig;
	void (*func)(QActive * const me, void *param);
	struct AUTORESPONSE *next;
}AUTORESPONSE;

typedef void (*pfThreadFree_t)(QActive *pstAct);
...
struct QMActive_s{
    ...
    */
	...
	uint8_t name[MAX_AO_NAME_LEN];

	AUTORESPONSE *listener_head;

	AUTORESPONSE *callback_head;

	pfThreadFree_t pfThreadFree;
...
* @include qf_start.c
*/
#define QACTIVE_START(me_, prio_, qSto_, qLen_, name_, stkLen_, param_) \
    ((*((QMActiveVtbl const *)((me_)->super.vptr))->start)( \
        (me_), (prio_), (qSto_), (qLen_), (name_), (stkLen_), (param_)))


#define QACTIVE_SET_FREE(me_, func_) (me_->pfThreadFree = func_)
...
/*! clear all event from the event queue of an active object. */
void QActive_flushAll_(QMActive * const me);
...
/*! Disarm a time event and recycle this event from active's queue */
bool QTimeEvt_disarmX(QTimeEvt * const me);
...
void QF_onCleanup(void);
    ...
    */
    ...
    void QF_publish_(QEvt const * const e, void const * const sender);
    #define QF_PUBLISH(e_, dummy_)   (QF_publish_((e_), (void const *)(dummy_)))
    ...
    */
    ...
    void QF_tickX_(uint_fast8_t const tickRate, void const * const sender);
    #define QF_TICK_X(tickRate_, dummy)   (QF_tickX_((tickRate_), (dummy)))
